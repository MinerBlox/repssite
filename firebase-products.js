...
function updateMeta(){ const count=document.getElementById("results-count"), copy=document.getElementById("results-copy"), pill=document.getElementById("active-filter-pill"); const total=totalForCurrent(); if(count) count.textContent=total==null?"Loading count…":`${total} item${total===1?"":"s"}`; if(pill) pill.textContent=selectedBrand?`Brand: ${selectedBrand}`:`Category: ${selectedCategory}`; if(copy){ if(searchTerm.trim()) copy.textContent=`Showing results for “${searchTerm.trim()}”.`; else if(selectedCategory!=="All") copy.textContent=`Showing ${selectedCategory}.`; else copy.textContent="Items load only as you scroll, with a two-row buffer."; } }

function renderChips(){ const wrap=document.getElementById("category-chips"); if(!wrap)return; let cats=filterCategories.filter(c=>c&&!hidden(c)); if(countsReady) cats=cats.filter(c=>Number(categoryCounts.get(c)||0)>0); wrap.innerHTML=["All",...new Set(cats)].map(c=>`<button class="category-chip ${selectedCategory===c?"active":""}" type="button" data-category="${esc(c)}">${esc(c)}</button>`).join(""); }
...
async function loadCounts(){ try{ const cached=JSON.parse(localStorage.getItem(CATEGORY_COUNT_CACHE_KEY)||"null"); if(cached?.savedAt&&Date.now()-cached.savedAt<CATEGORY_COUNT_CACHE_MS){categoryCounts=new Map(Object.entries(cached.counts||{}));countsReady=true;renderChips();updateMeta();return;} }catch{} const counts=new Map(); await Promise.all(filterCategories.map(async c=>{try{const s=await getCountFromServer(query(collection(db,"liveproducts"),where("category","==",c)));counts.set(c,s.data().count||0)}catch{counts.set(c,0)}})); categoryCounts=counts; countsReady=true; try{localStorage.setItem(CATEGORY_COUNT_CACHE_KEY,JSON.stringify({savedAt:Date.now(),counts:Object.fromEntries(counts)}))}catch{} renderChips();updateMeta(); }
async function loadCategories(){ try{ const s=await getDocs(collection(db,"categories")); const stored=s.docs.map(d=>d.data().name||d.id).filter(c=>c&&!hidden(c)); filterCategories=[...new Set([...FALLBACK_FILTER_CATEGORIES,...stored])]; }catch{} renderChips(); void loadCounts(); }

async function loadInitial(){ loadingPage=true; try{ const s=await getDocs(query(collection(db,"liveproducts"),orderBy("sortOrder","asc"),limit(PAGE_SIZE))); lastAllDoc=s.docs.at(-1)||null; allFinished=s.docs.length<PAGE_SIZE; firebaseItems=normalize(s.docs); }catch{ const s=await getDocs(query(collection(db,"liveproducts"),limit(PAGE_SIZE))); lastAllDoc=s.docs.at(-1)||null; allFinished=s.docs.length<PAGE_SIZE; firebaseItems=normalize(s.docs); }finally{loadingPage=false;} resetRendered(); document.getElementById("spreadsheet-loading")?.style.setProperty("display","none"); document.getElementById("product-grid")?.style.setProperty("display","grid"); }
async function loadNextPage(){ if(loadingPage||allFinished||selectedCategory!=="All"||searchTerm.trim()||selectedBrand)return; loadingPage=true; try{ const q=lastAllDoc?query(collection(db,"liveproducts"),orderBy("sortOrder","asc"),startAfter(lastAllDoc),limit(PAGE_SIZE)):query(collection(db,"liveproducts"),orderBy("sortOrder","asc"),limit(PAGE_SIZE)); const s=await getDocs(q); if(s.docs.length)lastAllDoc=s.docs.at(-1); if(s.docs.length<PAGE_SIZE)allFinished=true; merge(normalize(s.docs)); visibleItems=filtered(); appendUntilTarget(); }catch(e){ console.warn("Spreadsheet page load failed",e); }finally{loadingPage=false;} }
async function loadCategory(category){ selectedCategory=category; selectedBrand=""; searchTerm=""; const input=document.getElementById("search-input"); if(input)input.value=""; renderChips(); if(category==="All"){ resetRendered(); return; } if(categoryCache.has(category)){ firebaseItems=categoryCache.get(category); resetRendered(); return; } const grid=document.getElementById("product-grid"); if(grid)grid.style.opacity=".55"; try{ const s=await getDocs(query(collection(db,"liveproducts"),where("category","==",category))); const items=normalize(s.docs).sort((a,b)=>(Number(a.sortOrder)||0)-(Number(b.sortOrder)||0)); categoryCache.set(category,items); firebaseItems=items; resetRendered(); }catch(e){ console.warn("Category load failed",e); }finally{ if(grid)grid.style.opacity="1"; } }
...
const searchInput=document.getElementById("search-input"); if(searchInput){searchInput.value=searchTerm;searchInput.addEventListener("input",e=>{searchTerm=e.target.value;clearTimeout(searchTimer);searchTimer=setTimeout(async()=>{if(searchTerm.trim())await ensureFullForSearch();resetRendered();},220)})}
...
window.addEventListener("scroll",maybeLoadMore,{passive:true});window.addEventListener("resize",maybeLoadMore);
window.setCategory=loadCategory;window.clearCategory=()=>loadCategory("All");window.openCurrencyPicker=openCurrencyPicker;window.copyProductLink=async(url,id)=>{if(url&&url!=="#"){await navigator.clipboard.writeText(new URL(url,location.href).href);window.rcTrackProductInteraction?.(id,"copyClicks")}};

initializeCurrency();renderChips();void loadCategories();await loadInitial();if(searchTerm.trim()||selectedBrand){await ensureFullForSearch();resetRendered();}
