...
style.textContent=`
@media(min-width:721px){.category-row{overflow:visible!important}.category-chips{flex-wrap:wrap!important;overflow-x:visible!important}}
.image-loading-label{position:absolute;inset:0;display:grid;place-items:center;color:var(--muted);font-size:12px;font-weight:600;background:var(--surface2);z-index:0}
.image-loading-label.is-hidden{display:none}
.product-image{position:relative;z-index:1;opacity:0;transition:opacity .12s ease}
.product-image.is-loaded{opacity:1}
.category-loading-label{display:inline-flex;align-items:center;min-height:32px;padding:0 4px;color:var(--muted);font-size:12px;font-weight:600}
`;
...
function merge(items){const m=new Map(firebaseItems.map(i=>[i.id,i]));items.forEach(i=>m.set(i.id,i));firebaseItems=[...m.values()]}
...
function totalForCurrent(){if(searchTerm.trim()||selectedBrand)return filtered().length;if(selectedCategory!=="All"&&categoryCounts.has(selectedCategory))return Number(categoryCounts.get(selectedCategory)||0);if(allTotalCount!=null)return Number(allTotalCount||0);return Math.max(firebaseItems.length,visibleItems.length)}
function updateMeta(){const c=document.getElementById("results-count"),copy=document.getElementById("results-copy"),pill=document.getElementById("active-filter-pill"),total=totalForCurrent();if(c)c.textContent=`${total} item${total===1?"":"s"}`;if(pill)pill.textContent=selectedBrand?`Brand: ${selectedBrand}`:`Category: ${selectedCategory}`;if(copy){if(searchTerm.trim())copy.textContent=`Showing results for “${searchTerm.trim()}”.`;else if(selectedCategory!=="All")copy.textContent=categoryLoading?`Loading ${selectedCategory}…`:`Browse ${selectedCategory}.`;else copy.textContent="Browse all items."}}
function setEmptyState(mode){const e=document.getElementById("empty-state");if(!e)return;const h=e.querySelector("h3"),p=e.querySelector("p");if(mode==="hidden"){e.style.display="none";return}if(mode==="loading"){if(h)h.textContent="Loading items";if(p)p.textContent="Getting this category ready…";e.style.display="block";return}if(h)h.textContent="No items found";if(p)p.textContent="Try changing the search term or selecting a different category.";e.style.display="block"}
function refreshEmptyState(){if(categoryLoading){setEmptyState("loading");return}if(visibleItems.length){setEmptyState("hidden");return}if(searchTerm.trim()){setEmptyState(searchLoading?"loading":"empty");return}if(selectedCategory!=="All"){setEmptyState("loading");return}setEmptyState("empty")}
function renderChips(){const w=document.getElementById("category-chips");if(!w)return;if(!categoriesReady){w.innerHTML=`<span class="category-loading-label">Loading filters...</span>`;return}const cats=filterCategories.filter(c=>c&&!hidden(c)&&Number(categoryCounts.get(c)||0)>0);w.innerHTML=["All",...new Set(cats)].map(c=>`<button class="category-chip ${selectedCategory===c?"active":""}" type="button" data-category="${esc(c)}">${esc(c)}</button>`).join("")}
...
function appendUntilTarget(){const g=document.getElementById("product-grid");if(!g||categoryLoading)return;const t=Math.min(visibleItems.length,targetCount());if(t>renderedCount){g.insertAdjacentHTML("beforeend",visibleItems.slice(renderedCount,t).map(card).join(""));renderedCount=t}updateMeta();refreshEmptyState()}
function resetRendered(){const g=document.getElementById("product-grid");if(!g)return;visibleItems=filtered();renderedCount=0;g.innerHTML="";appendUntilTarget();refreshEmptyState()}

async function loadCategories(){renderChips();let candidates=[...FALLBACK_FILTER_CATEGORIES];try{const s=await getDocs(collection(db,"categories"));const stored=s.docs.map(d=>d.data().name||d.id).filter(c=>c&&!hidden(c));candidates=[...new Set([...candidates,...stored])]}catch{}const counts=new Map();await Promise.all(candidates.filter(c=>!hidden(c)).map(async c=>{try{const s=await getCountFromServer(query(collection(db,"liveproducts"),where("category","==",c)));counts.set(c,s.data().count||0)}catch{counts.set(c,0)}}));categoryCounts=counts;filterCategories=candidates.filter(c=>Number(counts.get(c)||0)>0);categoriesReady=true;allTotalCount=[...counts.values()].reduce((a,b)=>a+Number(b||0),0);renderChips();updateMeta()}

async function loadInitial(){loadingPage=true;try{const s=await getDocs(query(collection(db,"liveproducts"),orderBy("sortOrder","asc"),limit(PAGE_SIZE)));lastAllDoc=s.docs.at(-1)||null;allFinished=s.docs.length<PAGE_SIZE;firebaseItems=normalize(s.docs)}catch{const s=await getDocs(query(collection(db,"liveproducts"),limit(PAGE_SIZE)));lastAllDoc=s.docs.at(-1)||null;allFinished=s.docs.length<PAGE_SIZE;firebaseItems=normalize(s.docs).sort((a,b)=>(Number(a.sortOrder)||0)-(Number(b.sortOrder)||0))}finally{loadingPage=false}resetRendered();document.getElementById("spreadsheet-loading")?.style.setProperty("display","none");document.getElementById("product-grid")?.style.setProperty("display","grid")}
async function loadNextPage(){if(loadingPage||allFinished||selectedCategory!=="All"||searchTerm.trim()||selectedBrand)return;loadingPage=true;try{const q=lastAllDoc?query(collection(db,"liveproducts"),orderBy("sortOrder","asc"),startAfter(lastAllDoc),limit(PAGE_SIZE)):query(collection(db,"liveproducts"),orderBy("sortOrder","asc"),limit(PAGE_SIZE)),s=await getDocs(q);if(s.docs.length)lastAllDoc=s.docs.at(-1);if(s.docs.length<PAGE_SIZE)allFinished=true;merge(normalize(s.docs));firebaseItems.sort((a,b)=>(Number(a.sortOrder)||0)-(Number(b.sortOrder)||0));visibleItems=filtered();appendUntilTarget()}catch{}finally{loadingPage=false}}
async function getFullCategory(category){let lastError=null;for(let attempt=0;attempt<2;attempt++){try{const s=await getDocs(query(collection(db,"liveproducts"),where("category","==",category)));return normalize(s.docs).sort((a,b)=>(Number(a.sortOrder)||0)-(Number(b.sortOrder)||0))}catch(e){lastError=e;if(attempt===0)await new Promise(r=>setTimeout(r,250))}}throw lastError}
async function loadCategory(category){categoryRequestToken++;const token=categoryRequestToken;selectedCategory=category;selectedBrand="";searchTerm="";searchLoading=false;const input=document.getElementById("search-input");if(input)input.value="";renderChips();if(category==="All"){categoryLoading=false;await loadInitial();return}categoryLoading=true;firebaseItems=[];visibleItems=[];renderedCount=0;const grid=document.getElementById("product-grid");if(grid)grid.innerHTML="";updateMeta();refreshEmptyState();const cached=categoryCache.get(category);if(cached){if(token!==categoryRequestToken)return;firebaseItems=cached;categoryLoading=false;resetRendered();return}try{const items=await getFullCategory(category);if(token!==categoryRequestToken)return;categoryCache.set(category,items);categoryCounts.set(category,items.length);firebaseItems=items;categoryLoading=false;resetRendered()}catch{if(token===categoryRequestToken){categoryLoading=false;visibleItems=[];refreshEmptyState()}}}
async function ensureFullForSearch(){if(fullSearchLoaded)return;const s=await getDocs(collection(db,"liveproducts"));firebaseItems=normalize(s.docs).sort((a,b)=>(Number(a.sortOrder)||0)-(Number(b.sortOrder)||0));fullSearchLoaded=true}
async function maybeLoadMore(){if(renderFrame)return;renderFrame=requestAnimationFrame(async()=>{renderFrame=0;appendUntilTarget();if(selectedCategory==="All"&&!searchTerm.trim()&&!selectedBrand&&renderedCount>=visibleItems.length-2&&!allFinished){await loadNextPage();appendUntilTarget()}})}
...
if(searchInput){searchInput.value=searchTerm;searchInput.addEventListener("input",e=>{searchTerm=e.target.value;clearTimeout(searchTimer);searchTimer=setTimeout(async()=>{categoryRequestToken++;categoryLoading=false;if(searchTerm.trim()){searchLoading=true;firebaseItems=[];visibleItems=[];renderedCount=0;const grid=document.getElementById("product-grid");if(grid)grid.innerHTML="";refreshEmptyState();try{await ensureFullForSearch()}finally{searchLoading=false}}else{searchLoading=false;selectedCategory="All";await loadInitial();renderChips();return}resetRendered()},220)})}
...
void loadCategories();
await loadInitial();
if(searchTerm.trim()||selectedBrand){searchLoading=true;try{await ensureFullForSearch()}finally{searchLoading=false}resetRendered()}
