...
async function loadNextIndexedBatch(token=indexedRequestToken){if(indexedModeLoading||indexedModeCursor>=indexedModeIds.length)return;indexedModeLoading=true;try{const ids=indexedModeIds.slice(indexedModeCursor,indexedModeCursor+INDEX_BATCH_SIZE);const items=await loadItemsByIds(ids);if(token!==indexedRequestToken)return;indexedModeCursor+=ids.length;merge(items);visibleItems=[...firebaseItems];appendUntilTarget()}finally{indexedModeLoading=false}}
...
async function startIndexedMode({category="All",term="",brand=""}={}){await loadCatalogIndex();indexedRequestToken++;const token=indexedRequestToken;selectedCategory=category;searchTerm=term;selectedBrand=brand;indexedModeIds=matchingIndexIds();indexedModeCursor=0;firebaseItems=[];visibleItems=[];renderedCount=0;const g=document.getElementById("product-grid");if(g)g.innerHTML="";categoryLoading=category!=="All"&&!term;searchLoading=Boolean(term);renderChips();updateMeta();refreshEmptyState();if(!indexedModeIds.length){categoryLoading=false;searchLoading=false;refreshEmptyState();return}await loadNextIndexedBatch(token);if(token!==indexedRequestToken)return;categoryLoading=false;searchLoading=false;resetRendered()}
async function loadCategory(category){const input=document.getElementById("search-input");if(input)input.value="";if(category==="All"){indexedRequestToken++;selectedCategory="All";selectedBrand="";searchTerm="";categoryLoading=false;searchLoading=false;indexedModeIds=[];indexedModeCursor=0;await loadInitial();renderChips();return}await startIndexedMode({category,term:"",brand:""})}

async function maybeLoadMore(){if(renderFrame)return;renderFrame=requestAnimationFrame(async()=>{renderFrame=0;appendUntilTarget();const nearEnd=renderedCount>=visibleItems.length-2;if(!nearEnd)return;if(selectedCategory==="All"&&!searchTerm.trim()&&!selectedBrand){if(!allFinished)await loadNextPage();appendUntilTarget();return}if(indexedModeCursor<indexedModeIds.length){await loadNextIndexedBatch();appendUntilTarget()}})}
...
const searchInput=document.getElementById("search-input");if(searchInput){searchInput.value=searchTerm;searchInput.addEventListener("input",e=>{const term=e.target.value;clearTimeout(searchTimer);searchTimer=setTimeout(()=>{if(!term.trim()){void loadCategory("All");return}void startIndexedMode({category:"All",term,brand:""})},100)})}
